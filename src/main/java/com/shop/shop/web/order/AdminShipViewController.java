...
/**
 * 관리자 배송 View 핸들러 — 배송 시작(ship, 020) + 배송 완료(deliver, 021).
 *
 * <p>{@link AdminOrderViewController}는 {@code @RequestMapping("/admin/orders")}로
 * 클래스 레벨 매핑이 고정되어 있어 {@code /admin/shipments/{id}/ship}·{@code /deliver} 경로를
 * 같은 클래스에 넣으면 Spring MVC가 {@code /admin/orders/admin/shipments/{id}/...}으로
...
public class AdminShipViewController {
     ...
     * View는 절대 JSON을 반환하지 않는다 (error-response-rule, Constraint).
     *
     * <p>입력 파라미터 없음 — deliver는 shipmentId만으로 완료 처리 가능 (ship과 달리 carrier/trackingNumber 불요).
