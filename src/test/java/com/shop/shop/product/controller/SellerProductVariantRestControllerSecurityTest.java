...
class SellerProductVariantRestControllerSecurityTest {
    ...
    @Test
    @DisplayName("DELETE variant — SELLER(소유자) → 204")
    void deleteVariant_seller_returns_204() throws Exception {
        Product product = sampleProduct(SELLER_ID, PRODUCT_ID);
        ProductVariant variant = sampleVariant(VARIANT_ID, product);
        ...
        mockMvc.perform(delete("/api/v1/seller/products/" + PRODUCT_ID + "/variants/" + VARIANT_ID)
                        .header("Authorization", "Bearer " + sellerToken))
                .andExpect(status().isNoContent());
    }

    @Test
    @DisplayName("DELETE variant — ADMIN → 204 (RoleHierarchy 함의)")
    void deleteVariant_admin_returns_204() throws Exception {
        Product product = sampleProduct(SELLER_ID, PRODUCT_ID);
        ProductVariant variant = sampleVariant(VARIANT_ID, product);
    ...
    @Test
    @DisplayName("DELETE variant — CONSUMER → 403")
    void deleteVariant_consumer_returns_403() throws Exception {
        mockMvc.perform(delete("/api/v1/seller/products/" + PRODUCT_ID + "/variants/" + VARIANT_ID)
                        .header("Authorization", "Bearer " + consumerToken))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.status").value(403));
    ...
    @Test
    @DisplayName("DELETE variant — 비인증 → 401")
    void deleteVariant_unauthenticated_returns_401() throws Exception {
        mockMvc.perform(delete("/api/v1/seller/products/" + PRODUCT_ID + "/variants/" + VARIANT_ID))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.status").value(401));
    }
    ...
    @Test
    @DisplayName("DELETE variant — 타 판매자 → 404 (존재 은닉)")
    void deleteVariant_other_seller_returns_404() throws Exception {
    ...
    @Test
    @DisplayName("DELETE variant — 존재하지 않는 variant → 404")
    void deleteVariant_not_found_returns_404() throws Exception {
        Product product = sampleProduct(SELLER_ID, PRODUCT_ID);
        when(productRepository.findById(PRODUCT_ID)).thenReturn(Optional.of(product));
        when(productVariantRepository.findById(999L)).thenReturn(Optional.empty());

        mockMvc.perform(delete("/api/v1/seller/products/" + PRODUCT_ID + "/variants/999")
                        .header("Authorization", "Bearer " + sellerToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    }
